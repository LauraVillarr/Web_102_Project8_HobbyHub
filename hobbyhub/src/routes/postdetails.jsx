...
const PostDetails = () => {
    ...
    useEffect(() => {
        const getPost = async () => {
          ...
          .select()
          .eq('id', id);
          setPost({id: data[0].id, created_at: data[0].created_at, title: data[0].title, content: data[0].content, image_url: data[0].image_url, 
        ...
        <div className="post_details_page">
            <div className="post_details_main_content">
                <div className="post_details_card">
                    ...
                    <div className='post_details'>
                        ...
                        {post.image_url ? (
                            <img className='post_img' src={post.image_url} alt={'image for post ' + post.title} />
                        ) : null}
                    </div>
            ...
            <div className="post_details_suggested_posts">
                ...
                {randomPosts != null && randomPosts.length > 0 ? (
                            ...
                            <button onClick={(event) => moveToNewPostDetails(event, post.id)} className="suggested_post" key={index}>
                                <h5>{moment.utc(post.created_at).local().startOf('seconds').fromNow()}</h5>
